...
// Get member ID and month filter
$member_id = $_GET['id'];
$month = isset($_GET['bulan']) ? $_GET['bulan'] : null;

// Prepare filename
$filename = 'history_transaksi.csv';
...
fputcsv($output, array('Order ID', 'Tanggal', 'Status', 'Nomor Resi', 'Total'));

// Build SQL query based on member ID and month filter
...
while ($row = mysqli_fetch_assoc($result)) {
  ...
  // Write CSV row
  fputcsv($output, array($row['id'], $order_time, $row['confirm_payment'], $row['resi'], $row['total']));
}
